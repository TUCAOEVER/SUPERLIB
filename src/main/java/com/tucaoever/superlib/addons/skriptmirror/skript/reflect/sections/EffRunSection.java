package com.tucaoever.superlib.addons.skriptmirror.skript.reflect.sections;
...
public class EffRunSection extends Effect {
  ...
  private void runTask(Runnable task, boolean async) {
    if (async)
      Bukkit.getScheduler().runTaskAsynchronously(SUPERLIB.getInstance(), task);
    else
      Bukkit.getScheduler().runTask(SUPERLIB.getInstance(), task);
  }
