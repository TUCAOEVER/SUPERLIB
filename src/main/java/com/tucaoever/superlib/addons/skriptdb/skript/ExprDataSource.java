...
public class ExprDataSource extends SimpleExpression<HikariDataSource> {
    ...
    protected HikariDataSource[] get(Event e) {
        ...
        ds.setMaximumPoolSize(SUPERLIB.getInstance().getConfig().getInt("thread-pool-size", 10));
        // 30 minutes by default
        ds.setMaxLifetime(SUPERLIB.getInstance().getConfig().getInt("max-connection-lifetime", 1800000));
        //allow specifying of own sql driver class name
        if (!SUPERLIB.getInstance().getConfig().getString("sql-driver-class-name", "default").equals("default")) {
            ds.setDriverClassName(SUPERLIB.getInstance().getConfig().getString("sql-driver-class-name"));
        }
