...
public class EffCreateStaticHoloObject extends Effect
{
    ...
    protected void execute(final Event evt) {
            ...
            if (line.startsWith("ItemStack:")) {
                ...
                itemline.setTouchHandler((TouchHandler)new TouchHandler() {
                    public void onTouch(final Player player) {
                        final HoloTouchEvent event = new HoloTouchEvent(player, ((String)EffCreateStaticHoloObject.this.id.getSingle(evt)).replace("\"", ""), templine);
                ...
                textline.setTouchHandler((TouchHandler)new TouchHandler() {
                    public void onTouch(final Player player) {
                        final HoloTouchEvent event = new HoloTouchEvent(player, ((String)EffCreateStaticHoloObject.this.id.getSingle(evt)).replace("\"", ""), templine2);
                        Bukkit.getPluginManager().callEvent((Event)event);
                    }
                });
                ++lineNumber;
        ...
        textline2.setTouchHandler((TouchHandler)new TouchHandler() {
            public void onTouch(final Player player) {
                final HoloTouchEvent event = new HoloTouchEvent(player, ((String)EffCreateStaticHoloObject.this.id.getSingle(evt)).replace("\"", ""), templine3);
                Bukkit.getPluginManager().callEvent((Event)event);
            }
        });
