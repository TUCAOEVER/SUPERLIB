...
public class GUI {
	...
	public char convertSlot(int slot) {
		if (slot >= 0 && slot < getRawShape().length())
		...
		if (listener == null) {
			listener = new GUIListener(guiInventory) {
				...
				public void onOpen(InventoryOpenEvent e) {
					SUPERLIB.getGUIManager().setGUI((Player) e.getPlayer(), GUI.this);
				}

				@Override
				public void onClose(InventoryCloseEvent e) {
					if (!hasOnClose()) { // No way this event will be "cancelled"
						SUPERLIB.getGUIManager().removeGUI((Player) e.getPlayer());
					} else {
						SUPERLIB.getGUIManager().setGUIEvent(e, GUI.this);
						if (getOnClose() != null) {
							try {
								getOnClose().accept(e);
								if (isCloseCancelled()) {
									...
									}.runTaskLater(SUPERLIB.getInstance(), 1);
