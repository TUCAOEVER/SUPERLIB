...
public class EffEditHoloObject extends Effect
{
    ...
    public boolean init(final Expression<?>[] exp, final int arg1, final Kleenean arg2, final SkriptParser.ParseResult arg3) {
        ...
        this.text = (Expression<String>)exp[1];
        this.interactive = (Expression<Boolean>)exp[2];
        return true;
    }
        ...
        if (HoloManager.getFromHoloMap(((String)this.id.getSingle(evt)).replace("\"", "")) != null) {
                ...
                if (line.startsWith("ItemStack:")) {
                    ...
                    if (check) {
                        ...
                        itemline.setTouchHandler((TouchHandler)new TouchHandler() {
                            public void onTouch(final Player player) {
                                final HoloTouchEvent event = new HoloTouchEvent(player, ((String)EffEditHoloObject.this.id.getSingle(evt)).replace("\"", ""), templine);
                                Bukkit.getPluginManager().callEvent((Event)event);
                            }
                        });
                        itemline.setPickupHandler((PickupHandler)new PickupHandler() {
                            public void onPickup(final Player player) {
                                final HoloPickupEvent event = new HoloPickupEvent(player, ((String)EffEditHoloObject.this.id.getSingle(evt)).replace("\"", ""), templine);
                                Bukkit.getPluginManager().callEvent((Event)event);
                            }
                        });
                    }
                    ...
                    if (check) {
                        ...
                        textline.setTouchHandler((TouchHandler)new TouchHandler() {
                            public void onTouch(final Player player) {
                                final HoloTouchEvent event = new HoloTouchEvent(player, ((String)EffEditHoloObject.this.id.getSingle(evt)).replace("\"", ""), templine2);
                                Bukkit.getPluginManager().callEvent((Event)event);
                            }
                        });
                    }
            ...
            if (check) {
                ...
                textline2.setTouchHandler((TouchHandler)new TouchHandler() {
                    public void onTouch(final Player player) {
                        final HoloTouchEvent event = new HoloTouchEvent(player, ((String)EffEditHoloObject.this.id.getSingle(evt)).replace("\"", ""), templine3);
                        Bukkit.getPluginManager().callEvent((Event)event);
                    }
                });
            }
