...
public class EffCreateInteractiveStaticHolograms extends Effect
{
    ...
    protected void execute(final Event evt) {
            ...
            if (line.startsWith("ItemStack:")) {
                ...
                itemline.setTouchHandler((TouchHandler)new TouchHandler() {
                    public void onTouch(final Player player) {
                        final HoloTouchEvent event = new HoloTouchEvent(player, ((String)EffCreateInteractiveStaticHolograms.this.id.getSingle(evt)).replace("\"", ""), templine);
                        Bukkit.getPluginManager().callEvent((Event)event);
                    }
                });
                itemline.setPickupHandler((PickupHandler)new PickupHandler() {
                    public void onPickup(final Player player) {
                        final HoloPickupEvent event = new HoloPickupEvent(player, ((String)EffCreateInteractiveStaticHolograms.this.id.getSingle(evt)).replace("\"", ""), templine);
                        Bukkit.getPluginManager().callEvent((Event)event);
                    }
                });
                ++lineNumber;
                ...
                textline.setTouchHandler((TouchHandler)new TouchHandler() {
                    public void onTouch(final Player player) {
                        final HoloTouchEvent event = new HoloTouchEvent(player, ((String)EffCreateInteractiveStaticHolograms.this.id.getSingle(evt)).replace("\"", ""), templine2);
                        Bukkit.getPluginManager().callEvent((Event)event);
                    }
                });
                ++lineNumber;
        ...
        textline2.setTouchHandler((TouchHandler)new TouchHandler() {
            public void onTouch(final Player player) {
                final HoloTouchEvent event = new HoloTouchEvent(player, ((String)EffCreateInteractiveStaticHolograms.this.id.getSingle(evt)).replace("\"", ""), templine3);
                Bukkit.getPluginManager().callEvent((Event)event);
            }
        });
