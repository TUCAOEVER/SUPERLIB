...
import com.tucaoever.superlib.util.MathUtil;
...
public class NBTApi {
    ...
    private static void sendError(String error, Exception exception) {
        SUPERLIB.error("&cInvalid NBT: &7'&b" + error + "&7'&c");
        if (exception == null) return;
        SUPERLIB.error("&cCause: &e" + exception.getMessage());
    }
    ...
    private boolean forceLoadNBT() {
        ...
        try {
            ...
            SUPERLIB.log("&cFailed to load NBTApi!");
            return false;
        }
        SUPERLIB.log("&aSuccessfully loaded NBTApi!");
        return true;
    }
    ...
    public void setTag(@NotNull String tag, @NotNull NBTCompound nbtCompound, @NotNull Object[] object) {
        ...
        if (singleObject instanceof Boolean && isSingle) {
            ...
            SUPERLIB.error("Other-> KEY: " + key + " VALUE: " + singleObject + " VALUE-CLASS: " + object.getClass() + " TYPE: " + type);
        }
    }
